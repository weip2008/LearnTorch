...
cpdef int32_t identify_initial_pivot(double [:] X,
                                   double up_thresh,
                                   double down_thresh):
    cdef:
        double x_0 = X[0]
...
cpdef peak_valley_pivots_detailed(double [:] X,
    ...
    cdef:
        int32_t initial_pivot = identify_initial_pivot(X,
                                                     up_thresh,
                                                     down_thresh)
        int32_t t_n = len(X)
        ndarray[int32_t, ndim=1] pivots = np.zeros(t_n, dtype=np.int_)
        int32_t trend = -initial_pivot
        int32_t last_pivot_t = 0
...
@cython.boundscheck(False)
@cython.wraparound(False)
def pivots_to_modes(int32_t [:] pivots):
    """
    ...
    cdef:
        int32_t x, t
        ndarray[int32_t, ndim=1] modes = np.zeros(len(pivots),
                                                dtype=np.int_)
        int32_t mode = -pivots[0]
