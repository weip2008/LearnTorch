...
def df_slice_to_string(df):
    result = [
        (row['Normalized_Price'], row['Velocity'], row['Acceleration']) for _, row in df.iterrows()
    ]
    return str(result)
...
def generate_traintest_file(tddf_list, datatype):
    ...
    # Iterate over each tuple in tddf_highlow_list starting from the second tuple
        for i in range(0, len(tddf_list)):
            ...
            trainingdata_str = df_slice_to_string(traintestdata_df)
...
def process_data(start_date, end_date, datatype):
    ...
    if datatype != "Predict":
        ...
        slices_list  = parallel_cut_traintest_slice(ohlc_df, traintest_data_len+2, target_len, num_threads=6)

        #Here is the different part compare to ver. 5: shuffle the slices list
    
        random.seed(42)  # Set a seed for reproducibility
        random.shuffle(slices_list)
    else:
        # Use multi-threading to cut the DataFrame into slices
        slices_list  = parallel_cut_traintest_slice(ohlc_df, traintest_data_len+2, target_len, num_threads=1)
